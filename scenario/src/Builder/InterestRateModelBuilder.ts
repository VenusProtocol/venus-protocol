...
import { getAddressV, getExpNumberV, getPercentV, getStringV } from "../CoreValue";
...
export async function buildInterestRateModel(
  ...
  const fetchers = [
    ...
    new Fetcher<{ name: StringV; baseRate: NumberV; multiplier: NumberV }, InterestRateModelData>(
      ...
      async (world, { name, baseRate, multiplier }) => ({
        invokation: await WhitePaperInterestRateModel.deploy<InterestRateModel>(world, from, [
          baseRate.encode(),
          multiplier.encode(),
        ]),
        name: name.val,
    ...
    new Fetcher<
      { name: StringV; baseRate: NumberV; multiplier: NumberV; jump: NumberV; kink: NumberV },
    ...
    >(
      ...
      async (world, { name, baseRate, multiplier, jump, kink }) => ({
        ...
        description: `JumpRateModel baseRate=${baseRate.encode().toString()} multiplier=${multiplier
          .encode()
          .toString()} jump=${jump.encode().toString()} kink=${kink.encode().toString()}`,
        base: baseRate.encode().toString(),
        slope: multiplier.encode().toString(),
    ...
    new Fetcher<{ name: StringV; jump: NumberV; kink: NumberV; pot: AddressV; jug: AddressV }, InterestRateModelData>(
      ...
      async (world, { name, jump, kink, pot, jug }) => ({
        ...
        description: `DAIInterestRateModel jump=${jump.encode().toString()} kink=${kink.encode().toString()} pot=${
          pot.val
        } jug=${jug.val}`,
        jump: jump.encode().toString(),
        kink: kink.encode().toString(),
  ...
  const interestRateModelData = await getFetcherValue<any, InterestRateModelData>(
