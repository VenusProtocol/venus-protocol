  ...
  return [
    ...
    `,
      ...
      async (world, from, { newPendingAdmin, eta }) => {
        const invokation = await invoke(
          world,
          governor.methods.__queueSetTimelockPendingAdmin(newPendingAdmin.val, eta.encode()),
          from,
        );
    ...
    `,
      ...
      async (world, from, { newPendingAdmin, eta }) => {
        const invokation = await invoke(
          world,
          governor.methods.__executeSetTimelockPendingAdmin(newPendingAdmin.val, eta.encode()),
          from,
        );
