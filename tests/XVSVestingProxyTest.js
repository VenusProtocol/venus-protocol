...
describe('XVSVestingProxy', () => {
    ...
    describe("_setPendingImplementation", () => {
        ...
        describe("ZeroAddress as pending implementation", () => {
            it("does not change pending implementation address", async () => {
                await expect(send(xvsVestingProxy, '_setPendingImplementation', [ZERO_ADDRESS], { from: accounts[1] }))
    ...
    describe('_setPendingAdmin()', () => {
        it('should only be callable by admin', async () => {
            await expect(
                 send(xvsVestingProxy, '_setPendingAdmin', [accounts[0]], { from: accounts[0] })
    ...
    describe('_acceptAdmin()', () => {
        it('should fail when pending admin is zero', async () => {
            await expect( send(xvsVestingProxy, '_acceptAdmin')).rejects.toRevert("revert only address marked as pendingAdmin can accept as Admin");
        ...
        it('should fail on attempt to set zeroAddress as admin', async () => {
            expect(await send(xvsVestingProxy, '_setPendingAdmin', [accounts[0]])).toSucceed();
            await expect( send(xvsVestingProxy, '_setPendingAdmin', [ZERO_ADDRESS])).rejects.toRevert("revert Address cannot be Zero");
        ...
        it('should fail on multiple attempts of same address is set as PendingAdmin', async () => {
            expect(await send(xvsVestingProxy, '_setPendingAdmin', [accounts[0]])).toSucceed();
            await expect( send(xvsVestingProxy, '_setPendingAdmin', [accounts[0]])).rejects.toRevert("revert New pendingAdmin can not be same as the previous one");
        });
