...
describe('VRTVaultProxy', () => {
  ...
  describe("_setPendingImplementation", () => {
    describe("Check caller is admin", () => {
      it("does not change pending implementation address", async () => {
        await expect(send(vrtVaultProxy, '_setPendingImplementation', [vrtVault._address], { from: accounts[1] }))
  ...
  describe("_acceptImplementation", () => {
    it("Check caller is pendingImplementation  and pendingImplementation ≠ address(0) ", async () => {
      ...
      await expect(send(vrtVaultProxy, '_acceptImplementation', { from: root }))
  ...
  describe("Withdraw BEP20", () => {
    ...
    it("Admin can withdraw VRT", async () => {
      ...
      const withdrawBep20Txn = await send(vrtVaultProxy, "withdrawBep20", [vrtAddress, treasury, preFundedVRTInVault], { from: root });
      ...
      expect(new BigNum(tokenBalanceOfVaultAfterWithdrawal).isLessThan(new BigNum(tokenBalanceOfVaultBeforeWithdrawal))).toEqual(true);
      expect(new BigNum(tokenBalanceOfVaultAfterWithdrawal)).toEqual(new BigNum(tokenBalanceOfVaultBeforeWithdrawal).minus(preFundedVRTInVault));
    });
