...
describe('InterestRateModelLens', () => {
  ...
  describe('getSimulation', () => {
    it("must get simulation Data", async () => {
      ...
      const response = await call(interestRateModelLens, 'getSimulationResponse', [referenceAmountInWei, interestRateModelAddresses, reserveFactor]);
      ...
      const borrowRates_expected_data = fs.readFileSync(path.resolve(__dirname, './expected-data/borrow-rates-simulation.json'), 'utf8');
      ...
      const supplyRates_expected_data = fs.readFileSync(path.resolve(__dirname, './expected-data/supply-rates-simulation.json'), 'utf8');
