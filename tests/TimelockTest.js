const { encodeParameters, bnbUnsigned, freezeTime, keccak256 } = require("./Utils/BSC");
...
describe("Timelock", () => {
  ...
  beforeEach(async () => {
    ...
    queuedTxHash = keccak256(
      encodeParameters(["address", "uint256", "string", "bytes", "uint256"], [target, value, signature, data, eta]),
    );
  });
  ...
  describe("setDelay", () => {
    it("requires msg.sender to be Timelock", async () => {
      await expect(send(timelock, "setDelay", [delay], { from: root })).rejects.toRevert(
  ...
  describe("setPendingAdmin", () => {
    it("requires msg.sender to be Timelock", async () => {
      await expect(send(timelock, "setPendingAdmin", [newAdmin], { from: root })).rejects.toRevert(
  ...
  describe("acceptAdmin", () => {
    ...
    it("requires msg.sender to be pendingAdmin", async () => {
      await expect(send(timelock, "acceptAdmin", { from: notAdmin })).rejects.toRevert(
  ...
  describe("queue and cancel empty", () => {
    it("can queue and cancel an empty signature and data", async () => {
      const txHash = keccak256(
        encodeParameters(["address", "uint256", "string", "bytes", "uint256"], [target, value, "", "0x", eta]),
      );
  ...
  describe("executeTransaction (setDelay)", () => {
    ...
    it("requires timestamp to be greater than or equal to eta", async () => {
      ...
      ).rejects.toRevert("revert Timelock::executeTransaction: Transaction hasn't surpassed time lock.");
    });
