...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (network.name !== "hardhat") {
    ...
    await primeV2.transferOwnership(adminAccount[network.name]);

    console.log("Transferring PrimeLeaderboard ownership to Timelock...");
    await primeLeaderboard.transferOwnership(adminAccount[network.name]);

    console.log("Ownership transferred.");
  }
};

func.tags = ["ConfigurePrimeV2"];
func.id = "configure_prime_v2";
