...
[Slither](https://github.com/crytic/slither) runs on every PR and reports to the Security tab. It does not block merges.

Install the analyzer once:
...
yarn hardhat clean
SLITHER=true yarn hardhat compile
yarn slither

```

`SLITHER=true` keeps the sources generated by `hardhat-dependency-compiler`, which Slither needs to resolve the paths recorded in build-info. Clean first, because Slither reads every build-info file present and would otherwise analyze leftovers from an earlier compile.
