...
async function upgradeVault() {
    ...
    expect(await vault.methods.delegates('0xe8833153aE9171452855D391A1653766b70bdCcF').call())
    ...
    async function testVotingPower() {
        ...
        expect(await vault.methods.checkpoints(deployer, 0).call())
            .to.deep.include({ fromBlock: tx1.blockNumber.toString(), votes: '300000000000000000000000' });

        expect(await vault.methods.checkpoints(deployer, 1).call())
            .to.deep.include({ fromBlock: tx2.blockNumber.toString(), votes: '300002100000000000000000' });
    ...
    async function testWithdrawals() {
        ...
        expect(await vault.methods.getUserInfo(xvsAddress, 0, team).call())
            .to.deep.include({ amount: '300000000000000000000000', pendingWithdrawals: '2100000000000000000' });
        expect(await vault.methods.numCheckpoints(deployer).call()).to.equal('3');
        expect(await vault.methods.checkpoints(deployer, 2).call())
            .to.deep.include({ fromBlock: tx3.blockNumber.toString(), votes: '300000000000000000000000' });
        console.log('Checkpoints validated');
    }
