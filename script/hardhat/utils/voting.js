...
async function proposeAndVote({ gov, proposal, proposer, supporter }) {
    ...
    await gov.methods.propose(
        targets, values, signatures, getCalldatas(proposal), description
    ).send({ from: proposer });
    ...
    for (let i = 0; i < blocksToWait; ++i) {
        ...
        if (i % 1000 == 0) {
            console.log(
                `Waiting for the voting period to pass, ${(i / blocksToWait * 100).toFixed(2)}% completed`
            );
        }
    }
    console.log('Queuing...');
