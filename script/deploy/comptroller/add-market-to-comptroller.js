...
(async () => {
  ...
  console.log(`Comptroller -> Adding market for ${tokenSymbol} with Address: ${market} to Comtproller :${unitrollerAddress}`)
  ...
  if(txnEvents["Failure"]){
    ...
    console.error(`Failed to add ${tokenSymbol} to supportedMarkets. ErrorCode: ${errorCode} - ErrorDescription: ${errorDescription}`);
  }else{
    console.log(`Comptroller -> Added Market for: ${tokenSymbol} to Comptroller: ${unitrollerAddress} - with transactionStatus: ${addMarketToComptrollerTxn.status}`);
  }
